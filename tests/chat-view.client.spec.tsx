...
} from '@aihesa/ui-conversation-indexed/client'
...
describe('ChatView', () => {
  ...
  it('indexes each turn card and its rendered H1-H3 headings, then locates them', () => {
    const h = makeHarness({
      nodes: [
        user(1, '整理发布计划'),
        assistant(2, '# 总览\n\n## 准备\n\n### 检查项\n\n#### 不进入索引', 1),
    ...
    const view = render(<h.ChatView {...h.props} />)
    const index = view.getByRole('navigation', { name: '对话索引' })
    expect(within(index).getByRole('button', { name: '01整理发布计划' })).toBeTruthy()
    expect(within(index).getByRole('button', { name: '02补充回滚方案' })).toBeTruthy()
    expect(within(index).getByRole('button', { name: '总览' }).dataset.depth).toBe('1')
    expect(within(index).getByRole('button', { name: '准备' }).dataset.depth).toBe('2')
    expect(within(index).getByRole('button', { name: '检查项' }).dataset.depth).toBe('3')
    expect(within(index).queryByRole('button', { name: '不进入索引' })).toBeNull()
    expect(within(index).getByRole('button', { name: '回滚步骤' }).dataset.depth).toBe('2')

    const heading = view.getByRole('heading', { name: '检查项' })
