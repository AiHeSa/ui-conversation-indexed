...
- “加载更早”进入可见滚动区域时自动触发，并在历史内容插入后保持当前阅读位置；
- 可以隐藏索引，隐藏后保留紧凑的恢复入口，并将偏好保存到浏览器 localStorage；
- 索引内容可以独立滚动，但视觉上隐藏滚动条。

索引直接读取页面上已经渲染的语义 DOM，不会重新解析一遍 Markdown。因此 GFM、数学公式、流式输出和未来的 Markdown 行为仍以原渲染器为唯一实现。流式输出期间，索引会忽略普通正文变化，只响应轮次结构或 H1-H3 标题变化，并将重复扫描合并到每个动画帧最多一次，避免长对话的索引刷新阻塞正文渲染；隐藏索引后也会暂停流式 DOM 监听。
...
原有会话骨架、聊天消息行、编辑器、队列、审批界面、详情面板、会话统计、工具 slot、插件 slot 和流式输出行为均保持不变。历史分页仍保留语义滚动锚点，并增加上述可见时自动触发行为。ConversationController 只增加了上述原问题重发路径。
